...
class AFlowEdgeConfig(DirectionalTradingControllerConfigBase):
...
class FlowEdgeController(DirectionalTradingControllerBase):
    """
    FlowEdge Pro directional controller.
    ...
    def __init__(self, config: AFlowEdgeConfig, *args, **kwargs):
        self.config = config
        self.max_records_fast = config.fast_max_records
        self.max_records_slow = config.slow_max_records
        super().__init__(config, *args, **kwargs)
        self.logger().info(
            f"[FlowEdge-Pro] Started | pair={config.trading_pair} | "
    ...
    async def update_processed_data(self):
        ...
        entry_gate = "fast" if allow_fast_entry and slow_regime != MarketRegime.TRENDING else "slow"

        if slow_regime != MarketRegime.TRENDING and not allow_fast_entry:
    ...
    def _get_funding_bias(self) -> float:
        ...
        if "_perpetual" not in self.config.connector_name:
            return 0.0
        try:
            rate = self.market_data_provider.get_funding_info(
            ...
            if rate >= thr:
                return -0.15   # longs crowded → mild bearish tilt
            if rate <= -thr:
                return 0.15    # shorts crowded → mild bullish tilt
        except Exception:
            pass
        return 0.0
    ...
    def _self_adapt(self):
        """
        Embedded adaptive layer — observes own closed executor win rate
        and adjusts signal_threshold live. No external process needed.
        """
        closed = [e for e in self.executors_info if not e.is_active]
        if len(closed) < 3:
            return

        wins = sum(1 for e in closed if float(e.net_pnl_quote) > 0)
        win_rate = wins / len(closed)

        # Tighten threshold when losing too much
        if win_rate < 0.40 and self.config.signal_threshold < 0.75:
            self.config.signal_threshold = round(
                min(self.config.signal_threshold + 0.05, 0.75), 2
            )
            self.logger().warning(
                f"[FlowEdge-Pro] Win rate {win_rate:.0%} — "
                f"tightening threshold → {self.config.signal_threshold}"
            )

        # Loosen threshold when winning consistently
        elif win_rate > 0.65 and self.config.signal_threshold > 0.50:
            self.config.signal_threshold = round(
                max(self.config.signal_threshold - 0.02, 0.50), 2
            )
            self.logger().info(
                f"[FlowEdge-Pro] Win rate {win_rate:.0%} — "
                f"loosening threshold → {self.config.signal_threshold}"
            )

        realized_pnl = sum(float(e.net_pnl_quote) for e in closed)
        self.processed_data["win_rate"] = win_rate
        self.processed_data["closed_trades"] = len(closed)
    ...
    def to_format_status(self) -> List[str]:
        ...
        return [
            border,
            row("FlowEdge Pro - Directional DCA Controller"),
            border,
            row("Market State"),
            ...
            row(f"Threshold     : {self.config.signal_threshold:.2f}"),
